...
int main (int local_argc, char **local_argv) {
  ...
  unsigned long long sz = 1024*1024; // 1 MB unit
  unsigned long long cml_heap_sz = CML_HEAP_SIZE * sz;
  unsigned long long cml_stack_sz = CML_STACK_SIZE * sz;

  // Read CML_HEAP_SIZE env variable (if present)
  // Warning: strtoul may overflow!
  if(heap_env != NULL)
  {
    cml_heap_sz = strtoull(heap_env, &temp, 10);
    cml_heap_sz *= sz; //heap size is read in units of MBs
  }

  if(stack_env != NULL)
  {
    cml_stack_sz = strtoull(stack_env, &temp, 10);
  ...
  {
    #ifdef STDERR_MEM_EXHAUST
    fprintf(stderr,"Too small requested heap (%llu) or stack (%llu) size in bytes.\n",cml_heap_sz, cml_stack_sz);
    #endif
    exit(3);
  }
  ...
  {
    #ifdef STDERR_MEM_EXHAUST
    fprintf(stderr,"Too small requested heap (%llu) + stack (%llu) size in bytes.\n",cml_heap_sz, cml_stack_sz);
    #endif
    exit(3);
  }
